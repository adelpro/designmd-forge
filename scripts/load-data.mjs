...
if (!existsSync(stagingDir)) {
  console.error("No .staging/ directory found. Run \"npm run refresh-data\" (or an individual refresh-data:<source>) first.");
  process.exit(1);
}

const staged = readdirSync(stagingDir).filter((f) => f.endsWith(".json")).sort();
if (staged.length === 0) {
  console.error("No staged data found in .staging/. Run \"npm run refresh-data\" (or refresh-data:<source>) first.");
  process.exit(1);
}
