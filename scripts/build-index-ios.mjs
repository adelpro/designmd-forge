...
const index = [];
const categoryFolders = readdirSync(rootDir, { withFileTypes: true }).filter((d) =>
  d.isDirectory()
);
...
writeFileSync(
  ...
  JSON.stringify(
    { source: 'https://github.com/meliwat/awesome-ios-design-md', license: 'MIT', designs: index },
    null,
    2
  ),
  'utf-8'
...
console.log(
  `[ios] ${index.length} iOS app designs staged (all framework flavors copied). Next: run "npm run load-data".`
);
