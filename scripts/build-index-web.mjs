...
for (const line of lines) {
  ...
  if (catMatch) {
    if (
      /^(What is|Request a|AI Design|Collection|What's Inside|How to Use|Contributing|License)/.test(
        catMatch[1]
      )
    ) {
...
for (const entry of readmeEntries) {
  ...
  const longDescription =
    extractFrontmatterDescription(content) || extractPlainMarkdownDescription(content);
...
writeFileSync(
  ...
  JSON.stringify(
    { source: 'https://github.com/VoltAgent/awesome-design-md', license: 'MIT', designs: index },
    null,
    2
  ),
  'utf-8'
...
console.log(
  `[web] ${index.length} web designs staged (${missing} missing). Next: run "npm run load-data".`
);
